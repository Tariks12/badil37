...
adminApplyBtn.addEventListener('click', () => {
    console.log("Cheat code attempt...");
    const code = adminInput.value.trim().toUpperCase();
    if (code === 'NEON2026') {
        gold = 1000000;
        localStorage.setItem('neon_maxLevel', 5);
        saveStats();
        loadMenu();
        showGameNotification("HİLE AKTİF: 1M ALTIN VE TÜM SEVİYELER!", 5000);
        playSfx(1200, 'square', 0.5, 0.4);
        adminInput.value = '';
    } else {
        showGameNotification("GEÇERSİZ KOD!", 2000);
...
function init() {
    console.log("GAME INITIALIZED - V2");
    canvas.width = window.innerWidth; canvas.height = window.innerHeight;
